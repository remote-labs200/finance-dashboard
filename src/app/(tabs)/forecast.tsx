...
import { generateForecast, ForecastResult } from '@/lib/forecast-service';
        ...
        <ScrollView
          ...
          {forecast && forecast.forecasts.length > 0 && (
            <>
              ...
              <View style={[styles.summaryCard, { borderColor: colors.cardBorder }]}>
                <View style={styles.summaryRow}>
                  <View style={styles.summaryItem}>
                    <ThemedText type="small" themeColor="textSecondary">Avg Monthly Income</ThemedText>
                    <ThemedText type="headline" style={{ color: colors.success }}>
                      {formatCurrency(forecast.averageIncomeCents, 'USD')}
                    </ThemedText>
                  </View>
                  <View style={styles.summaryItem}>
                    <ThemedText type="small" themeColor="textSecondary">Avg Monthly Expenses</ThemedText>
                ...
                <View style={styles.trendRow}>
                  <ThemedText type="small" themeColor="textSecondary">Trend: </ThemedText>
                  <ThemedText
                    type="small"
                    style={{
                      color: forecast.trendDirection === 'up' ? colors.success
                        : forecast.trendDirection === 'down' ? colors.danger
                        : colors.warning,
                      fontWeight: '600',
                    }}>
                    {forecast.trendDirection === 'up' ? 'Growing' :
                     forecast.trendDirection === 'down' ? 'Declining' : 'Stable'}
              ...
              {/* Monthly Forecast */}
              <ThemedText type="callout" style={styles.sectionTitle}>Monthly Forecast</ThemedText>
              {forecast.forecasts.map((point) => {
                const monthNum = parseInt(point.month.split('-')[1], 10);
                const net = point.predictedIncomeCents - point.predictedExpenseCents;
                return (
                  <View key={point.month} style={[styles.forecastCard, { borderColor: colors.cardBorder }]}>
                    <View style={styles.forecastHeader}>
                      <ThemedText type="callout" style={{ fontWeight: '600' }}>
                        {getMonthName(monthNum)} {point.month.split('-')[0]}
                      </ThemedText>
                      <View style={[styles.confidenceBadge, {
                        backgroundColor: point.confidence > 0.7 ? colors.success + '26' : colors.warning + '26',
                      }]}>
                        <ThemedText type="small" style={{
                          color: point.confidence > 0.7 ? colors.success : colors.warning,
                        }}>
                    ...
                    <View style={[styles.forecastRow, styles.forecastNet, { borderTopColor: colors.divider }]}>
                      <ThemedText type="callout" style={{ fontWeight: '600' }}>Net</ThemedText>
                      <ThemedText type="callout" style={{
                        fontWeight: '600',
