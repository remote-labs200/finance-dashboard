...
export default function ScanScreen() {
  ...
  const pickImage = useCallback(async () => {
    ...
    if (!permission.granted) {
      Alert.alert(
        "Permission required",
        "Photo library permission is needed to select receipts.",
      );
      return;
    }
  ...
  const processImage = useCallback(async (uri: string, base64?: string) => {
    ...
    try {
      ...
      if (extraction) {
        ...
        Alert.alert(
          "Extraction failed",
          "Could not extract data from the receipt. Please try again or enter manually.",
        );
      }
    } catch {
  ...
  const saveAsTransaction = useCallback(async () => {
    ...
    const amountCents = Math.round(
      (receiptData.total ?? receiptData.amount ?? 0) * 100,
    );
    if (amountCents === 0) {
        ...
        >
          ...
          {!imageUri && (
            <View style={styles.actions}>
              <NeumorphicPressable onPress={takePhoto} style={styles.actionBtn}>
                <SymbolView
                  name={{
                    ios: "camera.fill",
              ...
              <NeumorphicPressable onPress={pickImage} style={styles.actionBtn}>
                <SymbolView
                  name={{
          ...
          {/* Extracted data */}
          {receiptData && !loading && (
            <NeumorphicCard style={styles.resultsCard}>
              <ThemedText type="callout" style={styles.resultsTitle}>
                Extracted Data
              </ThemedText>
          ...
          {!isAIEnabled() && !imageUri && (
            ...
            >
              ...
              <View style={styles.infoContent}>
                ...
                <ThemedText type="small" themeColor="textSecondary">
                  Configure EXPO_PUBLIC_SUPABASE_URL and deploy Edge Functions
