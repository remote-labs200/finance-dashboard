...
export default function SafeMonthlyPayScreen() {
  ...
  const persistTarget = useCallback(
    (val: string) => {
      ...
      if (user)
        setPreference(db, user.id, "smoothing_target_pct", sanitized || "70");
    },
    [user, db],
  );
  ...
  const persistBuffer = useCallback(
    (val: string) => {
      ...
      if (user)
        setPreference(db, user.id, "smoothing_buffer_months", sanitized || "3");
    },
    [user, db],
  );
  ...
  const persistMinPay = useCallback(
    (val: string) => {
      ...
      setMinPay(sanitized);
      if (user)
        setPreference(db, user.id, "smoothing_min_pay", sanitized || "0");
    },
    [user, db],
  );
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <ThemedText style={styles.description} themeColor="textSecondary">
        ...
        available for monthly living expenses vs. held for taxes and dry months.
      </ThemedText>
      ...
      >
        ...
        <NeumorphicCard style={styles.card}>
          ...
          <View style={styles.barOuter}>
            <View
              style={[
                styles.barFill,
                { width: `${targetNum}%`, backgroundColor: theme.primary },
              ]}
            />
          </View>
        ...
        <NeumorphicCard style={styles.card}>
          ...
          <ThemedText
            ...
            style={styles.parameterDesc}
          >
            How many months of expenses to keep in reserve for periods with no
            client payments.
          </ThemedText>
